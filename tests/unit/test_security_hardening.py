...
def test_sanitize_sensitive_string_redaction():
    ...
    raw_error = f"Error connecting to https://generativelanguage.googleapis.com/v1beta/models/gemini-flash:generateContent?key={mock_token}"
    sanitized = sanitize_sensitive_string(raw_error)
    assert mock_token not in sanitized
    assert "[REDACTED_API_KEY]" in sanitized

    raw_header_err = "Failed with x-goog-api-key: dummy_mock_secret_9999 and Bearer mock_bearer_token_123456789"
